...
class PortalAgreement(CustomerPortal):
    def _prepare_home_portal_values(self, counters):
        ...
        if "agreement_count" in counters:
            ...
            agreement_count = (
                agreement_model.search_count(self._get_filter_domain())
    ...
    def portal_my_agreements(self, page=1, date_begin=None, date_end=None, sortby=None):
        values = self._prepare_portal_layout_values()
        agreement_obj = request.env["agreement"]
        # Avoid error if the user does not have access.
        try:
            agreement_obj.check_access("read")
        except AccessError:
            return request.redirect("/my")
        domain = self._get_filter_domain()
        searchbar_sortings = {
            "date": {"label": request.env._("Date"), "order": "start_date desc"},
    ...
    def portal_my_agreement_accept(
        ...
        try:
            ...
            return {"error": request.env._("Invalid Agreement.")}

        if agreement_sudo.signature:
            return {"error": request.env._("The agreement is already signed.")}
        if not signature:
        ...
        agreement_sudo.write(
            {
                "signed_contract": pdf,
                "signed_contract_filename": f"{agreement_sudo.name}.pdf",
            }
        )
        agreement_sudo.message_post(
            author_id=(
                agreement_sudo.partner_id.id
                if request.env.user._is_public()
                else request.env.user.partner_id.id
            ),
            body=request.env._("Agreement signed by %s", name),
            attachments=[(f"{agreement_sudo.name}.pdf", pdf)],
