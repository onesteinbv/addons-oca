...
class TestSaleTimesheetLineNonBillable(common.TransactionCase):
    ...
    def test_write_invoiced(self):
        """This test aims to check whether the module allows to update non_billable field for already invoiced timesheet"""
        timesheet1 = self.SudoAccountAnalyticLine.create(
            {
                "project_id": self.task.project_id.id,
                "task_id": self.task.id,
        ...
        with self.assertRaisesRegex(
            ValidationError, "You can not modify timesheets in a way that would"
        ):
            timesheet1.write({"is_non_billable": True})

    def test_1(self):
        """This test aims to check whether the module allows to update non_billable field for already invoiced timesheet alongwith other calculations related to billing"""
        timesheet1 = self.SudoAccountAnalyticLine.create(
            {
                "project_id": self.task.project_id.id,
                "task_id": self.task.id,
