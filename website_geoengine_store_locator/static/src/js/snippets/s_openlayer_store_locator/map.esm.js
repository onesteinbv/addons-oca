...
class OpenLayerMap {
    constructor(element, mapType = "mapnik") {
        ...
        if (mapElement) {
            const popupElement = element.querySelector("#popup");
            new Popover(popupElement, map);
            
            const searchElement = element.querySelector("#search");
            const searchContainer = element.querySelector(".search");
            const flexdatalist = $(searchContainer).find(".flexdatalist-multiple");
            flexdatalist.remove();
            new Search(
                searchElement,
                map,
