...
class ProductProduct(models.Model):
    ...
    can_be_used_for_fleet = fields.Boolean(
        help="Helps to define whether this product is to be used for fleet or not",
    )
    uom_id_domain = fields.Binary(
        string="UOM Domain",
        help="Dynamic domain used for the uom that can be set on product to be"
        " used for fleet",
    ...
    def _onchange_can_be_used_for_fleet(self):
        if self.can_be_used_for_fleet:
            if self.uom_id not in [
                self.env.ref("uom.product_uom_km"),
                self.env.ref("uom.product_uom_mile"),
            ]:
