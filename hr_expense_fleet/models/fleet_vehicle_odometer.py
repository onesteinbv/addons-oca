...
class FleetVehicleOdometer(models.Model):
    ...
    distance = fields.Float(
        "Distance For Single Way Trip", help="Defines the distance covered for the trip"
    )
    is_roundtrip = fields.Boolean(help="Defines whether it is a round trip or not")
    total_distance = fields.Float(
        compute="_compute_total_distance",
        help="Defines the total distance covered including round trip",
    )
    is_private_trip = fields.Boolean(help="Defines whether it is a private trip or not")
    value = fields.Float(
        "Odometer End Value",
        group_operator="max",
        copy=False,
    ...
    expense_id = fields.Many2one(
        "hr.expense", "Expense", help="Defines expense record for this trip", copy=False
    )
    ...
    status = fields.Selection(
        [
            ("not_to_expense", "Not To Expense"),
            ("to_expense", "To Expense"),
            ("expense_created", "Expense Created"),
        ],
        help="Defines expense status for this trip",
        default="to_expense",
        copy=False,
    )
    ...
    def _onchange_vehicle(self):
        ...
        return super()._onchange_vehicle()
    ...
    def action_create_expense(self):
        ...
        to_expense_odometers = self.filtered(
            ...
            and (
                o.driver_employee_id == self.env.user.employee_id
        ...
        for product in products:
            ...
            total_distance = sum(
                line.total_distance
                for line in odometers.filtered(lambda ol: ol.unit == product_unit)
            )
            ...
            odometers.write(
                {"expense_id": hr_expense_rec.id, "status": "expense_created"}
            )
        return True
