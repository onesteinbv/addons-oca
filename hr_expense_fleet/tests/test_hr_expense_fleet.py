...
class TestHrExpenseFleet(SingleTransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_01_onchange_product_can_be_used_for_fleet(self):
        self.assertEqual(
            self.product_expense.uom_id, self.env.ref("uom.product_uom_unit")
        )
        ...
        self.assertEqual(
            self.product_expense.uom_id_domain, [("id", "in", [uom_km.id, uom_mile.id])]
        )
    ...
    def test_01_onchange_odometer(self):
        ...
        self.assertEqual(
            self.odometer_in_mi.product_id, self.vehicle_with_mi_odometer.product_id
        )
